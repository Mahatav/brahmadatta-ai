...
```
browser ───────> nginx ┬─ /                             → Astro (dev server, or static build)
...
| CSRF trusted origins must include the browser origin | Development uses `https://localhost:8443`; finale uses `http://localhost:8080` |

nginx sets, on every proxied request:

```
Host              $http_host      # verbatim, WITH the port — $host would drop :8443 / :8080
...
| browser origin | `https://localhost:8443` | `http://localhost:8080` |
| CSP | loosened for HMR | strict |
| HSTS | not sent | not sent |
...
**Finale** — plain loopback HTTP at `http://localhost:8080/`. This is deliberate. The
stage browser runs on the same machine as the stack, and the operator must not lose time
to a self-signed certificate interstitial. The finale profile therefore publishes only
`127.0.0.1:8080:8080`, mounts no TLS material, does not redirect to HTTPS, and does not
send HSTS. HSTS on `localhost` is especially dangerous because browsers key it by host,
not by port; one accidental header can poison later plain-HTTP rehearsal runs.

If judges must view the system from another machine, screen-share the stage browser or put
a trusted external reverse proxy in front of the host. Do not make the committed finale
compose file serve a self-signed `localhost` certificate.
...
| TLS certificate | $0 | $0 | None on the committed finale path; development self-signed material is local only |
| Domain | $0 | $0 | None registered; the finale is reached at `http://localhost:8080/` |
