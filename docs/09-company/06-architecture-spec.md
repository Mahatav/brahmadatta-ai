...
|---|---|
| `TRIAGE` (`ANALYZE`) | **Reopened under D-144; partially real as of D-151/#23's merge.** Compiler-warning capture (#23) is real: `workers/baseline/dispatch.py::_persist_compiler_diagnostics` turns real gcc/clang warnings from the BASELINE build into `AnalyzerTool.COMPILER_DIAGNOSTIC` `Finding` rows **during `BASELINE`, one stage before `TRIAGE`** — a byproduct of the build BASELINE already runs for the D3 gate, not a second scan (see that file's own docstring). Semgrep (#22, PR #274) is built and cybersecurity-reviewed but **not yet merged to `main`** as of this entry — `JobKind.ANALYZE` will back `TRIAGE` for real once it lands; until then `TRIAGE` itself still only emits `STAGE_STARTED`/a `LOG` event/`STAGE_COMPLETED` and runs no scan of its own. `git bisect` (#24, merged) is built and tested but **deliberately not part of this stage or any automatic mission path** — D-151 rules it a separate, operator-triggered capability (only meaningfully invokable against the #5 fixture today), never dispatched by `JOB_BACKED_STATES`. Git-history/bisect-timeline UI (#26) is unbuilt and depends on that not-yet-built bisect endpoint/executor actually emitting real events — see D-151 Question 1/3. The old "must not fabricate a finding count" discipline still applies: a real scan that finds nothing is a real zero, not the same as "no analyzer ran." |
...
| `GateName.RENEWED_FUZZING` | **No longer always `NOT_RUN`** — #40/D-144 (2026-08-24, merged) built this gate for real: `orchestrator/verification.py::run_verification` runs a bounded libFuzzer campaign (`RenewedFuzzConfig`, reusing `adapters.cpp.fuzzing.run_libfuzzer_campaign`, the same function `JobKind.FUZZ` uses) against the patched build and PASSes/FAILs on whether it finds a new crash; it still reports `NOT_RUN` with a disclosed reason when no `SANDBOX_FUZZ_IMAGE` is configured for the deployment or the harness itself could not be built/run. `derive_verdict`'s optional-gate `FAIL` branch is therefore live, not dead code, for this gate specifically — `STATIC_DELTA` alone still exercises only the `NOT_RUN` path. |
| `AnalyzerTool.COMPILER_DIAGNOSTIC` | **Produced as of #23/D-144 (merged)** — real gcc/clang `warning`/`error` diagnostics from the `BASELINE` stage's own build, structurally parsed (`adapters/cpp/compiler_diagnostics.py`), deduplicated against another tool's `Finding` on the same `(file_path, line)`, `DiscoveryMethod.STATIC_ANALYSIS`. Reuses `TRIAGE`'s originally-reserved `AnalyzerTool` slot but is recorded during the `BASELINE` job, one stage earlier — see the row above. The same-line dedup direction that actually fires today is "compiler defers to nothing yet," since Semgrep (#22) has not yet merged; once it does, ordering reverses per that PR's own module docstring. |
