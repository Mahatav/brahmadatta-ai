...
|---|---|
| `TRIAGE` (`ANALYZE`) | **D-144 (2026-08-24) reopened the CUT milestone; #23 (compiler-diagnostic capture) is built** — `workers/baseline/dispatch.py::_persist_compiler_diagnostics` turns real gcc/clang warnings from the BASELINE build into `AnalyzerTool.COMPILER_DIAGNOSTIC` `Finding` rows (during `BASELINE`, not `TRIAGE` — see below) plus a `StageToolRun` recording the compiler identity. Semgrep (#22) and git-history/bisect (#24/#26) remain not yet landed as of this entry; `TRIAGE` itself still emits `STAGE_STARTED`/a `LOG` event/`STAGE_COMPLETED` and does not itself run a scan (`JobKind.ANALYZE`, #22's own addition, is what will back `TRIAGE` once merged) — it must not fabricate a finding count beyond what BASELINE already recorded. |
...
| `AnalyzerTool.COMPILER_DIAGNOSTIC` | **Produced as of #23/D-144** — real gcc/clang `warning`/`error` diagnostics from the `BASELINE` stage's own build, structurally parsed (`adapters/cpp/compiler_diagnostics.py`), deduplicated against another tool's `Finding` on the same `(file_path, line)`, `DiscoveryMethod.STATIC_ANALYSIS`. Note this reuses `TRIAGE`'s originally-reserved `AnalyzerTool` slot but the findings are recorded during the `BASELINE` job, one stage earlier than `TRIAGE` — the compiler diagnostics are a byproduct of the build BASELINE already runs for the D3 gate, not a second scan. See `workers/baseline/dispatch.py`'s module docstring for the full reasoning, including the ordering note on why the same-line dedup direction that actually fires today is "compiler defers to nothing yet" until #22 (Semgrep) lands. |
