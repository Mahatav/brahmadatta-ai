...
| **PR #119 — `feat/authorize-snapshot`** | **PASS.** SEC-26, SEC-27, SEC-28, SEC-31 fixed at `b01755b` and independently re-verified (§15.16) — including the Postgres-specific savepoint claim, reproduced both with and without the fix. SEC-29, SEC-30 remain open, gated on #12/upload, unchanged. No Critical, no open condition on this PR. Round-4 pass in **§15**. |
...
### 15.16 Re-verification — 2026-08-08 · commit `b01755b` · SEC-26/27/28/31 closed

Re-verified **by name**, per the requested scope, not a fresh full pass. New worktree at
`b01755b`, fresh venv built from the pinned requirements, and a second isolated
`postgres:16-alpine` container (port 15433, removed after use). `git diff 3230585 b01755b`
read in full before testing anything.

**SEC-26 — closed, confirmed against my exact original PoC, not a re-description of it.**
...
file, same bytes, byte-identical to what I demonstrated the exploit with:
...
Refused, not silently accepted. I also independently disabled the new check (not the
author's added test, the check itself) and confirmed `test_a_tar_symlink_with_a_safe_name_
...
**SEC-27 — closed, and the specific savepoint claim reproduced both ways, not taken on
report.** Ran the exact cross-mission race from §15.5 (two different missions, byte-identical
content, forced interleave via a barrier around `Artifact.objects.filter(...).first()`)
against the fixed code on a **fresh, real Postgres container**:
...
claimed, and it does not hold on SQLite (which has no equivalent poisoned-transaction
behavior) — it is a Postgres-specific failure mode that only a real Postgres run could have
caught, which is what makes re-running this myself rather than reading the diff the right
call here. Restored the savepoint, re-ran the race clean, then ran the full suite: 299
passed.

**SEC-28 — the 403 is the right call, and I am recording that as a deliberate ruling, not
letting it ride through with the code-naming fix.** `RepositoryOutOfScopeError` now carries
`ErrorCode.UNSUPPORTED_REPOSITORY` with `http_status` kept at `403`, and the class docstring
states the reason explicitly: this is a scope/permission refusal ("you may not point this
endpoint at this reference at all" — the allowlist boundary CLAUDE.md and this module's own
docstring both call a safety boundary) rather than a content-validity refusal (the archive was
read and failed to parse, which is `UnreadableArchiveError`'s `422`). I agree with that
distinction. `403 Forbidden` is conventionally an access-control status, and "this reference is
outside what this deployment is authorized to read, independent of who is asking" is an
access-control decision in substance even though the *identity* asking is not what is being
judged — the same shape `AuthorizationScopeError` in the same file already uses at `403` for
an analogous reason. I considered whether `422` (matching `UnreadableArchiveError`'s status,
if not its code) or `404` (avoid confirming/denying the path's existence) would be better
fits and reject both: `422` would blur exactly the distinction this fix's docstring is making
between "can't parse what we found" and "not allowed to look there at all," and `404` is wrong
here because the refusal is not about existence, it's about scope, and the response already
says so honestly rather than pretending not to know. **Ruling: 403 is correct. Not a new
finding.** Independently injection-verified: reverted the code (not the status) to
`INVALID_AUTHORIZATION`, confirmed `test_repository_ref_with_an_unsupported_scheme_is_refused`
goes red (`'INVALID_AUTHORIZATION' == 'UNSUPPORTED_REPOSITORY'` assertion failure), restored
via `git checkout`, confirmed green.

**SEC-31 — closed, and the adjacent fix is accurate, not a broadening of scope beyond what I
should sign off on.** The two false citations are gone, replaced with two tests I confirmed
exist by name (`test_snapshot_with_an_expired_authorization_is_refused`,
...
"Demonstrated at exactly one stage: `INGEST`" — matching my own §15.12 finding word for word
in substance. Confirmed `api/routers/missions.py` was not touched by this commit
...
reviewed this adjacent change because it touches the same file and the same property
("is the enforcement claim accurate"), not because I am expanding this round's scope — it is
a direct tightening of a claim adjacent to the one I originally flagged, and it is accurate.

**The author's own account — spot-checked, not merely accepted.** They report every fix was
checked by injecting the violation and confirming red before green. I independently
re-injected all four myself, using my own constructions rather than re-running theirs where
it mattered most (SEC-26: my original PoC file, unmodified; SEC-27: my own real-Postgres
harness, not their SQLite-monkeypatch regression test; SEC-28: a direct revert of the
`code =` line). All four went red on injection and green on restoration, independently.
**The standard held.**

**Updated verdict: PASS.** All four merge conditions from §15.14 (SEC-26, SEC-27, SEC-28,
SEC-31) are closed and independently re-verified. SEC-29 and SEC-30 remain open, correctly
untouched, gated on #12 and the future upload endpoint respectively — unchanged from §15.14.
SEC-32 remains INFO, no action required. No Critical, no open condition on this PR.
**PR #119 clears security review outright**, subject only to the two forward-looking gates
already on record (SEC-29 before #12, SEC-30 before an upload endpoint).
